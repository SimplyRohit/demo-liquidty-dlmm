...
export function setupUserCommands(
  ...
  bot.command("userpositions", async (ctx) => {
    try {
      ...
      if (positions.length === 0) {
        await ctx.reply(
          `No positions found for user ${userPubKeyStr} in pool ${poolAddr}.`
        );
        return;
      }
      ...
      const positionInfo = positions
        .map((position: PositionInfo, index: number) => {
  ...
  Position Account: ${position.position.toString()}`;
      ...
      await ctx.reply(
        `Error fetching user positions: ${String((err as Error)?.message ?? err)}`
      );
    }
  });
}
